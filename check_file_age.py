...
def thresholds(filename, timedelta, warning, critical):
    ...
    message = '- %s is %s old |current_age=%s;warning=%s;critical=%s' %(filename, timedelta, timedelta, warning, critical)
...
def check(filename, method, warning, critical):
    ...
    thresholds(filename=filename, timedelta=timedelta, warning=warning, critical=critical)
